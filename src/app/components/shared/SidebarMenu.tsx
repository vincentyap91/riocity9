...
// --- Configuration ---
const rewardItems = [
    { label: "Spin Wheel Bonus", icon: Dices, path: "/bonus/wheel", color: "text-yellow-400" },
    { label: "Voucher Scratch Bonus", icon: Ticket, path: "/bonus/scratch", color: "text-pink-400" },
    { label: "Prize Box Bonus", icon: Box, path: "/bonus/prize", color: "text-orange-400" },
];

const menuItems = [
    { label: "Home", icon: Home, path: "/" },
    { label: "Hot Games", icon: Flame, path: "/hot" },
    { label: "All", icon: Gamepad2, path: "/all" },
    { label: "Sports", icon: Trophy, path: "/sports" },
    { label: "Live Casino", icon: Dices, path: "/live-casino" },
    { label: "Slots", icon: Gamepad2, path: "/slots" },
    { label: "Fish Hunt", icon: Fish, path: "/fishing" },
    { label: "Lottery", icon: Ticket, path: "/lottery" },
    { label: "Exchange", icon: ArrowLeftRight, path: "/exchange" },
    { label: "Poker", icon: Club, path: "/poker" },
    { label: "Crash", icon: Plane, path: "/crash" },
    { label: "Promotion", icon: Megaphone, path: "/promotions" },
    { label: "Referral", icon: Users, path: "/referral" },
    { label: "Rebate", icon: HandCoins, path: "/rebate" },
    { label: "Membership", icon: Crown, path: "/vip" },
];
    ...
    return (
        <div className="flex flex-col h-full bg-[#02040a] text-white overflow-y-auto custom-scrollbar">
                        ...
                        <AccordionTrigger className="px-4 py-4 hover:no-underline bg-[#2a2105] border-none [&>svg]:hidden group">
                            <div className="flex items-center gap-3 text-left w-full">
                                ...
                                <div className="flex flex-col flex-1">
                                    <span className="font-bold text-white text-[16px] leading-tight">Reward Centre</span>
                                    <span className="text-xs text-[#fab005] font-medium mt-0.5">3 Active Bonuses</span>
                                </div>
                            ...
                            <div className="flex flex-col py-2">
                                {rewardItems.map((item, idx) => (
                                    ...
                                    >
                                        ...
                                        <span className="font-medium text-[14px] text-white/90 group-hover:text-white">{item.label}</span>
                                    </Link>
                                ))}
                            </div>
                        </AccordionContent>
                ...
                {menuItems.map((item, idx) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <span className={`font-bold text-[15px] tracking-wide ${isActive ? 'text-[#00ff88]' : 'text-white'}`}>{item.label}</span>
            ...
            <div className="p-4 mt-auto space-y-4">
                ...
                {/* Live Chat Card */}
                <div className="bg-[#131b29] border border-white/5 rounded-2xl p-4 flex items-center justify-between group cursor-pointer hover:border-emerald-500/30 transition-all">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-full bg-[#00ff88] flex items-center justify-center">
                            <MessageCircle className="w-6 h-6 text-[#131b29] fill-[#131b29]" />
                        </div>
                        <span className="font-bold text-[#00ff88] text-lg">Live Chat</span>
                    </div>
                    <div className="bg-[#fa5252] text-white text-[11px] font-bold px-2 py-0.5 min-w-[24px] h-5 flex items-center justify-center rounded-md shadow-lg">
                        11
                    </div>
                </div>
